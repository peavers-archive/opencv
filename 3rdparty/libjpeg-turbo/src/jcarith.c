 ...
 * According to sections F.***REDACTED-IP***.3 and F.***REDACTED-IP***, we need at least
...
encode_mcu_DC_first(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    /* Sections F.1.4.1 & F.***REDACTED-IP***: Encoding of DC coefficients */
    ...
    if ((v = m - entropy->last_dc_val[ci]) == 0) {
      ...
      arith_encode(cinfo, st, 0);
      /* Section F.***REDACTED-IP***.2: Establish dc_context conditioning category */
...
encode_mcu_AC_first(j_compress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  /* Sections F.1.4.2 & F.***REDACTED-IP***: Encoding of AC coefficients */
